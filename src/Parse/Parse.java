...
public class Parse
{
  ...
  public MutablePair<ArrayList<Term>,int[]> fnParseText1 (StringBuilder sbText , String docName){
    ...
    for (int iIndex = 0, arrayStringWordsLength = arrStringWords.length; iIndex < arrayStringWordsLength; iIndex++)
    {
      ...
      {
        if(!Objects.equals("", mm = fnIsMonth(sWord))){//date parsing starts with month
            ...
            if(2>=dd.length() && 31>= S2Int(dd) && 0<S2Int(dd) && !dd.equals("")){
                if(iIndex+2<arrayStringWordsLength){
                    ...
                    if(0!=S2Int(yyyy)&&4==yyyy.length()){
                        ...
                        else{termMap.put(dd+"/"+mm+"/"+yyyy,new Term(dd+"/"+mm+"/"+yyyy,docName,1,iIndex,2));}
                        iIndex+=2;
                    }
                    else {if(termMap.containsKey(dd+"/"+mm)){
                        termMap.get(dd+"/"+mm).raisC();}
                        else{termMap.put(dd+"/"+mm,new Term(dd+"/"+mm,docName,1,iIndex,2));}
                        iIndex+=1;
                    }
                }
            }
            else if(0!=S2Int(dd)&& 4==dd.length()){//dd is yyyy
                     if(termMap.containsKey(mm+"/"+dd)){
                          termMap.get(mm+"/"+dd).raisC();}
                     else{termMap.put(mm+"/"+dd,new Term(mm+"/"+dd,docName,1,iIndex,2));}
                     iIndex+=1;
            }
            }
          }
          ...
          sWord=fnRemoveAllCharOfKind(sWord,',');
          sWord=fnCleanS(sWord);
          if (Objects.equals(sWord, "")) continue;
          if('$'==sWord.charAt(0)){
              ...
              else{termMap.put("$"+number,new Term("$"+number,docName,1,iIndex,6));}
          }
          else if(2==sWord.length()){// check is date
              dd=sWord;
              ...
              if('%'==sWord.charAt(sWord.length()-1)){//perecent
                  number=fnParseNumber(sWord.substring(0,sWord.length()-1));
                  if(Objects.equals(number, "")) continue;
                  if(hashSetStopWords.contains(number+" percent")){continue;}
                  if(termMap.containsKey(number+" percent")){
                      termMap.get(number+" percent").raisC();}
                  else{termMap.put(number+" percent",new Term(number+" percent",docName,1,iIndex,3));}
              }
              else{
...

  {
    ...
    while(sResult.length()>2&&sResult.charAt(0)=='0'&&sResult.charAt(1)!='.'){ sResult=sResult.substring(1);}
    ...
    if(iPoint!=-1){
        ...
        if(l>0||(iPoint==1&&sResult.charAt(0)=='0')){//l is good
            if(le.length()==0){
   ...
   *
   * @param s
   *     the string we want to delete from
   * @param c
   *     the char we want to delete
   *
